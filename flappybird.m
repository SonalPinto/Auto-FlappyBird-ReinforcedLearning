      ...
      %% Cycling the Palette
       ...
       %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        if(toc(prevTime) >= 0.01 || gameover)
            ...
            % Decide next action
            if (Q(dx,dy,2) > Q(dx,dy,1) && ~gameover)
                a = 2;
                FlyKeyStatus = true;
            else 
                a = 1;
...
%              display([h dx dy 2]);
            Q(ps(1),ps(2),ps(3)) = 0.5*Q(ps(1),ps(2),ps(3)) ...
                      + 0.5*( reward + 0.9 * max(Q(dx,dy,1), Q(dx,dy,2))); 
...
%         Bird.Angle = double(mod(CurrentFrameNo,360))*pi/180;
        ...
        if ShowFPS
            ...
            if mod(total_frame_update,SHOWFPS_FRAMES) == 0 % If time to update fps
                ...
                set(fps_text_handle, 'String',sprintf('sx.sy: %d, %d',ps(2), ps(3)));
                FPS_lastTime = toc(stageStartTime);
            end
            set(var_text_handle, 'String', sprintf('Deaths/HighScore: %d / %d', deaths, highScore));
        end
    end
    if fall_to_bottom
        if Score > Best
